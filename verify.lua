...
local handle, error_message = fs.open("disk/auth.txt", "r") -- "r" means "read"
...
local auth = handle.readAll() -- read all data in the file
handle.close() -- very important to close filehandles!

local temp = string.len(player) * balance
if(to_base64(player .. temp)) == auth then
    print("Card Owner: " .. player)
    print("Verified")
else
    print("Unverified")
end